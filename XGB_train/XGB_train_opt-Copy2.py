...
path_to_NTuples="/nfs/dust/cms/user/makou/BBH_Tuples/BBH_synchtuples/2016_post/"

columns=["m_sv", "m_vis", "pt_1", "pt_2", "puppimet","nbtag","njets","bcsv_1","bcsv_2","bpt_1","mbb","dRbb", "jpt_1","jpt_2","pt_tt" ,"jdeta","pzeta" ,"iso_1", "iso_2","extraelec_veto","extramuon_veto","q_1","q_2","gen_match_1" ,"gen_match_2","dr_tt","evt","run","os","qcdweight", "gen_nbjets","weightEMu", "trg_muhigh_elow","trg_ehigh_mulow", "gen_noutgoing","gen_nbjets_cut" ]
...
general_cut= 'pzeta>-35 and pt_2>15 and pt_1>15 and iso_1<0.15 and iso_2<0.2 and extraelec_veto<0.5 and extramuon_veto<0.5 and dr_tt> 0.3 and ((pt_1 >24 and trg_ehigh_mulow > 0.5 ) | (pt_2 >24 and trg_muhigh_elow > 0.5)) and nbtag<=3'
...
bbhsig=pd.concat([arr_all["bbHToTauTau_ybyt_M125"], arr_all["bbHToTauTau_yt2_M125"], arr_all["bbHToTauTau_yb2_M125"]], axis=0)
bbHsig=bbhsig.query('os >0.5')
print('bbHsig',bbHsig.shape)
bbh= bbHsig.loc[:,'m_sv':'jdeta']
bbH=bbh.to_numpy()
label_bbh = 4*(np.ones(len(bbH)))
Higgs_sig = np.column_stack ((bbH,label_bbh))
print("Higgs_sig",Higgs_sig.shape)



ggHsig=(arr_all["GluGluHToTauTau_M125"])[(arr_all["GluGluHToTauTau_M125"])["os"]>0.5][(arr_all["GluGluHToTauTau_M125"])["gen_nbjets_cut"]==0]
print('ggHsig',ggHsig.shape)

VBFsig=(arr_all["VBFHToTauTau_M125"])[(arr_all["VBFHToTauTau_M125"])["os"]>0.5]
print("VBFsig", VBFsig)
#ZHsig = (arr_all["ZHToTauTau_M125_13TeV"])[(arr_all["ZHToTauTau_M125_13TeV"])["os"]>0.5]
#Whsig=pd.concat([arr_all["WplusHToTauTau_M125"], arr_all["WminusHToTauTau_M125"]], axis=0)
#WHsig= Whsig.query('os >0.5

H_bkg= (pd.concat([ggHsig,VBFsig],axis=0)).loc[:,'m_sv':'jdeta'] 
Higgs_bkg= H_bkg.to_numpy()
label_Higgs_bkg= np.ones(len(Higgs_bkg))
Higgs_Bkg = np.column_stack ((Higgs_bkg, label_Higgs_bkg))
print("Higgs_Bkg",Higgs_Bkg.shape)

QCD=pd.concat([arr_all["MuonEG_Run2016F"], arr_all["MuonEG_Run2016G"], arr_all["MuonEG_Run2016H"]] , axis=0)
qcd=QCD.query('os<0.5')

DB= pd.concat([arr_all["WZTo2L2Q"], arr_all["WZTo3LNu"], arr_all["ZZTo2L2Q"], arr_all["ZZTo4L"], arr_all["WWTo2L2Nu"]], axis=0)
Diboson=DB.query('os>0.5')

st=pd.concat([arr_all["ST_t-channel_antitop_4f"] ,arr_all["ST_t-channel_top_4f"] ,arr_all["ST_tW_antitop_5f"],arr_all["ST_tW_top_5f"]],axis=0)
ST=st.query('os>0.5')

wjets=pd.concat([arr_all["W1JetsToLNu"] ,arr_all["W2JetsToLNu"] ,arr_all["W3JetsToLNu"],arr_all["W4JetsToLNu"], arr_all["WJetsToLNu"]],axis=0)
WJets=wjets.query('os>0.5')

mix =(pd.concat([Diboson,ST,WJets,qcd],axis=0).loc[:,'m_sv':'jdeta'])
misc = mix.to_numpy()
...
Ztt= DYJets.loc[:,'m_sv':'jdeta']
Z=Ztt.to_numpy()
label_ZTT = 2*(np.ones(len(Z)))
ZTT = np.column_stack ((Z, label_ZTT))
print("ZTT_shape",ZTT.shape)
print("ZTT", ZTT)
Data= np.concatenate((ttbar,Higgs_Bkg,ZTT,miscelle,Higgs_sig),axis=0)  
print('Data',Data)
...
X = scaler0.fit_transform(Data_ran[:,0:16])
print('X',X)
print('shape of x',X.shape)
np.save('X0_16post_UL.npy', X)    # .npy extension is added if not given
pickle.dump(scaler0, open('scaler0_16post_UL.pkl', 'wb'))
...
# save to JSON
model.save_model("model0_16post_UL.json")
# save to text format
model.save_model("model0_16post_UL.txt")
# save
...
cm_df = pd.DataFrame(cmn,
                     index = ['ttbar','Higgs_Bkg','ZTT','misc','Higgs_sig'], 
                    columns = ['ttbar','Higgs_Bkg','ZTT','misc','Higgs_sig'])

sns.heatmap(cm_df, annot=True)
plt.title('Confusion Matrix')
plt.ylabel('Actual Values')
plt.xlabel('Predicted Values')
name=('CM_test_model0_2016post_UL')
...
visualizer= ROCAUC(model, classes=["ttbar","Higgs_Bkg","ZTT","misc","Higgs_sig"])
visualizer.fit(X_train,y_train)
visualizer.score(X_test,y_test)
plt.legend()
name=('ROC_AUC_test0_2016post_UL')
plt.savefig('XGBoosted_output/{}'.format(name))
...
pyplot.legend()
# show the plot
name=('loss-function_test_model0_2016post_UL')
plt.savefig('XGBoosted_output/{}'.format(name))
...
name=('Feature Importance test 2016 post UL')
plt.savefig('XGBoosted_output/{}'.format(name))
